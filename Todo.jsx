...
function Todo() {
    ...
    function submitHandler() {
        if (!text) {
            ...
            if (edit) {
                ...
                //npm install uuid     **required to import 
                const { v4: uuidv4 } = require('uuid')
                let data = {
                    id:uuidv4(),   //performance.now()   ,//Math.trunc(Math.random() * 10000),  //Date.now()
                    text           // same as [text: text]   this is an es8 feature if key an value is same the we rite only one
                }
                setArr((pre) => {
                    return [...pre, data]
                })
    ...
    return (
        <div className="container-fluid">
            ...
            <CardPrint arr={arr} deleteHandler={deleteHandler} editHandler={editHandler} />
        </div>
    )
}
